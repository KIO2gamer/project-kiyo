    ...
    async execute(interaction) {
        ...
        try {
            if (options in imagePaths) {
                await interaction.channel.send({ content: caption, files: [imagePaths[options]] });
                console.log(`Image posted by ${interaction.user.tag}: ${options}`);
                await interaction.reply({ content: `Image successfully posted: ${options}`, ephemeral: true });
            } else {
                await interaction.reply({ content: "There is no such option available", ephemeral: true });
            }
        } catch (error) {
