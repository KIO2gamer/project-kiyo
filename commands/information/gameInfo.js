		...
		if (gameName) {
			try {
				...
				const response = await fetch(
					`https://api.rawg.io/api/games?key=${process.env.RAWG_API_KEY}&search=${encodeURIComponent(game.name)}`
				);
				const data = await response.json();

				const gameDetailsResponse = await fetch(
					`https://api.rawg.io/api/games/${data.results[0].id}?key=${process.env.RAWG_API_KEY}`
				);
				...
				const embed = new EmbedBuilder()
					...
					.addFields(
						{
							name: 'Developer',
							value: gameDetails.developer || 'Unknown',
							inline: true,
						},
						{
							name: 'Publisher',
							value: gameDetails.publisher || 'Unknown',
							inline: true,
						},
						{
							name: 'Release Date',
							value: data.results[0].released || 'Unknown',
							inline: true,
						},
						{
							name: 'Positive Reviews',
							value: gameDetails.positive.toString(),
							inline: true,
						},
						{
							name: 'Negative Reviews',
							value: gameDetails.negative.toString(),
							inline: true,
						},
				...
				await interaction.editReply(
					'An error occurred while fetching the game information. Please try again later.'
				);
			}
		}
	},
