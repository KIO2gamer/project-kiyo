...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (timestamps && now < timestamps + cooldownAmount) {
            ...
            return interaction.reply(`Please wait ${timeLeft.toFixed(1)} more seconds before using the \`summon\` command again.`);
        }
        ...
        try {
            await interaction.channel.send({ content: `https://tenor.com/view/cat-spiritus-summon-vintage-fountain-pen-gif-22872604` });
            await interaction.channel.send({ embeds: [embed] });
            cooldowns.set(interaction.user.id, now);
            console.log(`${interaction.user.tag} summoned ${userOption.tag} at ${new Date(now).toISOString()}`);
        } catch (error) {
