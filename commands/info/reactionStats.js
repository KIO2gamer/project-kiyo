...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Displays statistics on reactions given in a specific channel or server-wide.')
    ...
    async execute(interaction) {
        ...
        const startDate = moment().subtract(parseInt(timeframe.slice(0, -1)), timeframe.slice(-1)).toDate();
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Reaction statistics from ${channel ? `<#${channel.id}>` : 'the server'} over the past ${timeframe}`)
            .addFields([
                { name: 'Top Reactions', value: sortedReactions.map(([emoji, count]) => `${emoji}: ${count}`).join('\n') || 'No reactions' },
                { name: 'Top Reactors', value: sortedUsers.map(([user, count]) => `<@${user}>: ${count}`).join('\n') || 'No reactors' }
            ])
            .setTimestamp();
