...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const activities = guildUser.presence && guildUser.presence.activities.length
            ? guildUser.presence.activities.map(activity => `${activity.type}: ${activity.name}`).join(', ')
            : 'None';
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Flags:', value: `\`${userFlags.length ? userFlags.map(flag => flag.replace(/_/g, ' ')).join('\n') : 'None'}\``, inline: true },
