...
function success(interaction, { title, description, color = COLORS.success, fields = [] }) {
  return base(interaction).setTitle(title).setDescription(description).setColor(color).addFields(fields);
}
...
function dmNotice({ guildName, title, description, color = COLORS.info }) {
  return new EmbedBuilder().setTitle(title || `Notice from ${guildName}`).setDescription(description).setColor(color).setTimestamp();
}
