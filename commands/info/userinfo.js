...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const badges = userFlags.map(flag => badgeEmojis[flag] || flag.replace(/_/g, ' ').toLowerCase()).join(' ');
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Flags:', value: `\`${userFlags.length ? userFlags.map(flag => flag.replace(/_/g, ' ')).join(', ') : 'None'}\``, inline: true },
        ...
        if (guildUser.premiumSince) {
            embed.addFields({ name: 'Boosting Since:', value: `<t:${parseInt(guildUser.premiumSinceTimestamp / 1000)}:R>`, inline: true });
        }

        embed.addFields(
            { name: 'Status:', value: `${guildUser.presence ? guildUser.presence.status : 'offline'}`, inline: true },
            { name: 'Activities:', value: `${guildUser.presence ? guildUser.presence.activities.map(activity => activity.name).join(', ') : 'None'}`, inline: true }
        );
