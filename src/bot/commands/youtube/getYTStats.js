...
module.exports = {
  ...
  usage:
    '/youtube_stats <channel_link / channel_id / channel_handle / video_link>',
  examples: [
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!channelData) {
        return interaction.editReply('No channel found with the provided ID.');
      }

      const latestVideoData = await getLatestVideoData(youtube, channelId);
...
function createFields(channelData, latestVideoData) {
  return [
    ...
    {
      name: 'Published',
      value: new Date(latestVideoData.latestVideoPublishedAt).toDateString(),
      inline: true,
    },
    {
      name: 'Views',
