...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const oauthData = await getAuthorizationDataFromMongoDB(interactionId);
      if (
          ...
          [
            {
              ...
              value:
                "Please make sure you have connected your YouTube account to Discord and try again.",
              inline: false,
            },
          ],
...
async function assignSubscriberRole(member, subscriberCount) {
  ...
  const roleName = roleRanges.find((range) => subscriberCount < range.max).name;
