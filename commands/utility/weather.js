...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      const response = await axios.get(`https://api.weatherapi.com/v1/current.json?key=ea32f85feec2496aac1152011231112&q=${encodeURIComponent(location)}&aqi=no`);
      ...
      console.error(error);
      await interaction.reply({ content: 'An error occurred while fetching the weather data.', ephemeral: true });
    }
  },
};
