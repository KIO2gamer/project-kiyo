...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const detectedLanguage = detectResult.response ? detectResult.response.text() : "Unknown";

            // Translation
            const translateResult = await model.generateContent(promptTranslate);
            const translatedText = translateResult.response ? translateResult.response.text() : "Translation failed";

            const embed = new EmbedBuilder()
                .setTitle("Translation Result")
                .setColor("#00ff00")
                .addFields(
                    { name: "Detected Language    ------------------>", value: detectedLanguage, inline: true },
                    { name: "Target Language", value: target, inline: true },
                    { name: "\n", value: "\n", inline: true },
                    { name: "Input Text    ---------------------------->", value: input, inline: true },
                    { name: "Translated Text", value: translatedText, inline: true },
                    { name: "\n", value: "\n", inline: true },
                )
