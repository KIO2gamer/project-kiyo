...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (timestamps && now < timestamps + cooldownAmount) {
            ...
            return interaction.reply(`Please wait ${timeLeft.toFixed(1)} more seconds before using the \`snipe\` command again.`);
        }
        ...
        try {
            await interaction.channel.send({ content: `https://tenor.com/view/family-guy-peter-griffin-gun-point-sniper-rifle-gif-16445332` });
            await interaction.channel.send({ embeds: [embed] });
            cooldowns.set(interaction.user.id, now);
            console.log(`${interaction.user.tag} sniped ${userOption.tag} at ${new Date(now).toISOString()}`);
        } catch (error) {
