	...
	async execute(interaction) {
		fs.readFile('./assets/texts/hangmanWords.txt', 'utf-8', async (err, data) => {
			if (err) {
				...
				interaction.reply('An error occurred while loading the words.');
				return;
			}
			...
			const gameEmbed = new EmbedBuilder()
				...
				.addFields(
					{ name: 'Guesses Left', value: remainingGuesses.toString(), inline: true },
					{
						name: 'Guessed Letters',
						value: guessedLetters.join(', ') || 'None',
						inline: true,
					},
					{ name: 'Hints Used', value: hintsUsed.toString(), inline: true }
				);
			...
			collector.on('collect', async guess => {
				...
				if (content.startsWith('!hint') && hintsUsed < 3) {
					...
					if (guessedLetters.includes(letter)) {
						await guess.reply('You already guessed that letter!');
						return;
					}
					...
					await guess.reply('Please enter a single letter or use "!hint" for a hint.');
					return;
				}
				...
				gameEmbed
					...
					.setFields(
						{ name: 'Guesses Left', value: remainingGuesses.toString(), inline: true },
						{ name: 'Guessed Letters', value: guessedLetters.join(', '), inline: true },
						{ name: 'Hints Used', value: hintsUsed.toString(), inline: true }
					);
