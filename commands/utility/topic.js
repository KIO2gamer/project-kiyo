...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!userMentions) {
            return interaction.reply({ content: 'No users provided. Please specify at least one user to change the topic.', ephemeral: true });
        }

        await interaction.reply({ content: 'Topic change message sent.', ephemeral: true });
        await interaction.channel.send(`${userMentions}\n**Please change the topic immediately. Failing to do so will result in a mute/ban.**`);
    }
};
