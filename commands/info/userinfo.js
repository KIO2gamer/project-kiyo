...
module.exports = {
	description_full: 'Shows information about a user, either the user who executed the command or a specified user. This includes their username, ID, roles, join date, status, activity, and more.',
	...
	async execute(interaction) {
		...
		function getActivityName(activity) {
			switch (activity.type) {
				...
				case ActivityType.Custom:
					return activity.state ? `${activity.emoji} ${activity.state}` : 'Custom Status';
				default:
		...
		const fields = [
			...
			{
				name: 'Display Color',
				value: member.displayHexColor === '#000000' ? 'Default' : member.displayHexColor,
				inline: true,
			},
		];
