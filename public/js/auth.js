...
function encryptState(text, secretKey) {
...
const AuthManager = {
	...
	async handleLogin() {
		try {
			...
			const encryptedState = encryptState(rawState, process.env.STATE_ENCRYPTION_KEY);
			...
			window.location.href = `https://discord.com/oauth2/authorize?client_id=${encodeURIComponent(clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=identify%20connections&state=${encodeURIComponent(encryptedState)}`;
		} catch (error) {
