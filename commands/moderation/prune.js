...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const userMessages = fetchedMessages.filter(message => message.author.id === userId).first(amount);

            if (userMessages.length === 0) {
                return interaction.reply({ content: `No messages found from ${userOption.username}`, ephemeral: true });
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`Failed to prune messages from <@${userId}>\nError: \`${error.message}\``);
