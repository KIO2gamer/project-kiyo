...
module.exports = {
	...
	async execute(interaction) {
		...
		joinCollector.on('end', async () => {
			if (players.length < 2) {
				return await interaction.followUp('Not enough players to start. :(');
			}

			await interaction.followUp(`Game starting with ${players.length} players! Get ready!`);
		...
		async function playRound() {
			...
			try {
				...
				if (!songData) {
					await interaction.followUp("Oops, couldn't fetch a song. Try again later!");
					return;
				}
				...
				const roundTimeout = setTimeout(async () => {
					await interaction.followUp("Time's up! Nobody got the answer this round.");
				...
				answerCollector.on('end', async (collected, reason) => {
					...
					if (!hintGiven) {
						...
						const hintCollector = interaction.channel.awaitReactions({
						...
						hintCollector.on('collect', async (reaction, user) => {
							const requestingPlayer = players.find(p => p.id === user.id);

							if (requestingPlayer) {
								if (requestingPlayer.score > 0) {
									requestingPlayer.score -= HINT_PENALTY;
								}
								await interaction.followUp(`Hint:  ${provideHint(correctLine)}`);
								hintGiven = true;
							}
						});
...
function processLyrics(fullLyrics) {
	...
	if (lyricsLines.length < 3) {
		return { lyrics: 'The song is too short for this game!', correctLine: null };
	}
